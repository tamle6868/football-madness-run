...
export class GameScene extends Phaser.Scene {
  ...
  init(data: EndlessRunData = {}) {
    ...
    this.dailyChallengeProgress = Number(
      localStorage.getItem("devin-game-daily-progress") ?? 0
    );
    this.best = Number(localStorage.getItem("devin-game-best") ?? 0);
  }
  ...
  create() {
    ...
    this.superBeam = this.add.graphics().setDepth(12).setVisible(false);
  ...
  private endSuper() {
    ...
    this.particles.stop();
    this.superBeam.clear().setVisible(false);
    this.hideSpeech();
  ...
  private updateSuperBeam() {
    ...
    for (const obstacle of obstacleChildren) {
      ...
      const obstacleCoreY = obstacle.y - obstacle.displayHeight * 0.48;
      const beamYAtObstacle = sourceY + (obstacle.x - sourceX) * 0.015;
      const hitRange = Math.max(76, obstacle.displayHeight * 0.62);
      if (Math.abs(obstacleCoreY - beamYAtObstacle) > hitRange) continue;

      if (!obstacle.getData("beamHit")) {
  ...
  private triggerGameOver(obs?: Phaser.Physics.Arcade.Sprite) {
    ...
    if (finalScore > this.best) {
      ...
      localStorage.setItem("devin-game-best", String(this.best));
    }
    this.score = finalScore;

    // Update daily progress
    const newDaily = this.dailyChallengeProgress + Math.floor(this.distance);
    localStorage.setItem("devin-game-daily-progress", String(newDaily));

    // Save total coins
    const totalCoins = Number(localStorage.getItem("devin-game-coins") ?? 0) + this.coinsCollected;
    localStorage.setItem("devin-game-coins", String(totalCoins));
