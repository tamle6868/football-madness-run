...
export class CharacterSelectScene extends Phaser.Scene {
  ...
  create() {
    this.selectedId = getCharacter(
      localStorage.getItem("devin-game-character") ?? DEFAULT_CHARACTER_ID
    ).id;
    ...
    this.add
      .text(GAME_WIDTH / 2, 58, "CHOOSE YOUR CHARACTER", {
      ...
      .setOrigin(0.5);
    this.add
      .text(GAME_WIDTH / 2, 106, "Each character has a unique Super skill to survive the run.", {
        fontFamily: "sans-serif",
        fontSize: "18px",
        color: "#a0d0ff",
        fontStyle: "bold",
    ...
    this.makeButton(GAME_WIDTH / 2 - 120, 650, 230, 58, "START RUN", 0x32d264, () => {
      localStorage.setItem("devin-game-character", this.selectedId);
