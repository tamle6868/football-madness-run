...
export class MainMenuScene extends Phaser.Scene {
  ...
  create() {
    ...
    const bubble = this.add
      ...
      .setScale(0.8);
    const siuu = this.add
      .text(bubble.x, bubble.y - 14, "READY?", {
    ...
    this.add
      .text(GAME_WIDTH / 2, 230, "Run forever. Dodge everything. Chase the high score.", {
        fontFamily: "sans-serif",
        fontSize: "26px",
        color: "#fff8e0",
        fontStyle: "italic",
    ...
    const best = Number(localStorage.getItem("devin-game-best") ?? 0);
    this.add
    ...
    const lines = [
      "Pick a character with a unique Super skill",
      "Jump, slide, collect coins, fill the Meter",
      "Activate Super to break through obstacles",
    ];
